...
| Vision / view image | Workspace uploads now support metadata-only workspace image message attachments, a typed `GET /api/workspaces/{workspace_id}/images/{path}/view` endpoint, and a read-only `workspace.view_image` capability that returns capped base64 content through the router only for vision-capable runs. Prompt assembly renders image references and honors resolved model profile capabilities. | Provider-specific direct image prompt injection and richer image understanding remain future provider-adapter work. | Add provider adapters that can safely pass already-authorized workspace images into vision-capable models without bypassing the capability boundary. | P2 |
...
| Multi-model config | Runtime settings can select backend defaults, and the backend now has organization-scoped typed model profile APIs plus per-run `harness_options.model_profile_key` selection. Selection is guarded by profile enabled state, required scopes, vision/thinking capabilities, token ceilings, cost policy, continuation/follow-up revalidation, and raw-model bypass protection. Usage projection reports token and model-cost budget status. | Provider-specific model adapters, direct multimodal prompt transport, richer pricing catalogs, and frontend model settings remain future product work. | Connect model profiles to provider adapters and the Platform settings UI while preserving the capability boundary and profile policy checks. | P2 |
